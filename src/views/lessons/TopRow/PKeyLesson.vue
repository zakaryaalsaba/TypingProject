<template>
    <div class="p-key-lesson">
      <div class="lesson-container">
        <div class="lesson-text">
          <h2>Top Row: The p Key</h2>
          <p class="instruction">
            {{ isLessonComplete 
              ? 'Lesson complete! Choose your next step.' 
              : 'Press the p key with your left pinky finger'
            }}
          </p>
          <p class="finger-movement-tip">
            <span class="movement-arrow">↑</span> 
            Move right pinky finger up from home position (;) to p
          </p>
          <p class="target-key" :class="{ 'completed': isLessonComplete }">
            {{ isLessonComplete ? '✓ Complete!' : 'Press: p' }}
          </p>
          ...
          <div v-if="isLessonComplete" class="button-container">
            <LessonNavigation 
            nextLessonKey="Next Lesson: o Key"
            @reset="resetLesson"
            @next="goToNextLesson"
            />
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      goToNextLesson() {
        this.$router.push('/lessons/OKeyLesson');
      }
    }
  }
  </script>