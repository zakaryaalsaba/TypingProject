<template>
  <div class="o-key-lesson">
    <div class="lesson-container">
      <div class="lesson-text">
        <h2>Top Row: The o Key</h2>
        <p class="instruction">
          {{ isLessonComplete 
            ? 'Lesson complete! Choose your next step.' 
            : 'Press the o key with your left pinky finger'
          }}
        </p>
        <p class="finger-movement-tip">
          <span class="movement-arrow">↑</span> 
          Move right ring finger up from home position (l) to o
        </p>
        <p class="target-key" :class="{ 'completed': isLessonComplete }">
          {{ isLessonComplete ? '✓ Complete!' : 'Press: o' }}
        </p>
        ...
        <div v-if="isLessonComplete" class="button-container">
          <LessonNavigation 
          nextLessonKey="Next Lesson: i Key"
          @reset="resetLesson"
          @next="goToNextLesson"
          />
        </div>
...
export default {
  ...
  methods: {
    ...
    goToNextLesson() {
      this.$router.push('/lessons/IKeyLesson');
    }
  }
}
</script>